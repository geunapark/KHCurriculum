...
SELECT 1+2+3
FROM DUAL; --DUAL 테이블 사용법

SELECT LENGTH('HELLO') -- 몇글자인지 알고싶을 때 ,LENGTH()
FROM DUAL;

SELECT LENGTH(EMAIL) 
FROM EMPLOYEE;

SELECT LENGTHB('안녕') --LENGTHB 글자 바이트 수 한글은 3바이트 영어는 1바이트
FROM DUAL;
...
/*
    2) INSTR
        ...
        - -1은 뒤에서부터 찾기 시작 (몇번째 문자부터 탐색을 할건지)
*/
...
--B라는 단어를 첫번째 단어부터 탐색을 할건데 2번째 B가 어디있는지 찾아라
...
/* 
    3) LPAD / RPAD
        ...
        //문자열은 왼쪽 숫자열은 오른쪽으로 자동적으로 출력
*/

--20만큼의 길이중 EMAIL 값은 오른쪽으로 정렬하고 공백을 왼쪽으로 채운다
--20만큼의 길이 중 EMAIL 값은 왼쪽으로 정렬하고 공백을 오른쪽으로 채운다
SELECT
    EMAIL
    ,LPAD(EMAIL , 20 , '#')--공백을 #으로 채워줌
    ,RPAD(EMAIL , 20)
FROM EMPLOYEE;
...
SELECT RTRIM('KH   ') AS "X" 
FROM DUAL;
SELECT RTRIM('00012300004560000', '0' )
FROM DUAL;
...
-- (991212-2****** 형태로 출력)
SELECT EMP_NAME , EMP_NO ,SUBSTR(EMP_NO , 1 ,0) || '******' AS 주민번호
FROM EMPLOYEE;
...
SELECT EMP_NAME , EMAIL , SUBSTR(EMAIL , 1 , INSTR(EMAIL , '@' , 1)-1) AS 아이디
FROM EMPLOYEE;
...
SELECT TO_CHAR(SYSDATE, 'YYYY"년" MM"월"DD"일"DY"요일"') FROM DUAL;
...
SELECT 
    EMP_NAME
    , DEPT_CODE
    , DECODE (DEPT_CODE , 'D1' , '개발 1팀' , 'D2' ,'개발2팀') AS "부서명"
FROM EMPLOYEE;
...
--사원명 , 월급 , 초급/중급/고급/특급 구분 (급여가300까지는 초급 , 500까지는 중급 700까지는 고급 그외에는 특급)
SELECT
    EMP_NAME
    ,SALARY
    ,CASE 
