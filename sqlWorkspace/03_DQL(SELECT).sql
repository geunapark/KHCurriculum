...
/*
    ...
    --데이터를 조회할 때 사용
    -SELECT를 통해 얻은 결과를 RESULT SET 이라고 한다.(결과집합)
    -조회하려는 칼럼은 반드시 FROM절에 기술한 테이블에 존재해야한다
    
 */   
 
 -- EMPLOYEE 테이블에서 EMP_ID , EMP_NAME조회
 ...
 --EMPLOYEE  테이블에서 전체 사원의 모든 컬럼(*)  정보조회
 SELECT *
 FROM EMPLOYEE;
 ...
 */
 ...
 --직원명 , 연봉 , 보너스 ,보너스가 포함된 연봉((급여+(급혀*보너스))*12조회
 --***산술연산 중 NULL값이 존재 할 경우 , 결과값은 무조건 NULL
 ...
 /*
    <칼럼명에 별칭 지정>
        ...
        -사용 이유 : 산술연산 등으로 인해 칼럼명이 지저분해질 경우, 깔끔하게 작정하기 위해서 사용
 */ 
 
 
 /*
    <리터럴>
        SELECT 절에 리터럴을 사용하면 테이블에 조재하는 데이터처럼 조회가 가능하다.
...
--사번 , 사원명 , 급여 연결해서 조회
SELECT EMP_ID || EMP_NAME || '의 월급 '|| SALARY||'원' AS "사원 정보 및 급여"
FROM EMPLOYEE;
...
SELECT EMP_ID , EMP_NAME 
FROM EMPLOYEE
WHERE EMP_ID = 210;  --주의사항 : 동등비교연산자는 JAVA처럼  == 2개 X  = 1개 O
...
--실습문제
--1, EMT_NY칼럼값이 'N'인 사원들의 사번,이름,입사일 조회
 ...
 */
 ...
 --부서 코드가 D6이면서 급여가 300만원 이상인 직원들의 모든 칼럼 조회
SELECT *
FROM EMPLOYEE
WHERE DEPT_CODE = 'D6' AND SALARY >= 3000000;
 
 --급여가 400만원이상 직급코드가 J2인 사원의 모든 칼럼 조회
 SELECT *
 FROM EMPLOYEE
 WHERE DEPT_CODE = 'J2' AND SALARY >= 4000000;
 
 --부서코드가 D6이거나 D2에 해당하는 직원의 모든 칼럼
SELECT *
FROM EMPLOYEE
WHERE DEPT_CODE = 'D6' OR DEPT_CODE = 'D2';

--급여가 350만원 이상 600만원 이하 인 직원의 이름 부서코드 급여조회
SELECT EMP_NAME , DEPT_CODE , SALARY
FROM EMPLOYEE
...
--입사일이 90/01/01 ~ 01/01/01인 사원의 모든 칼럼 조회
SELECT *
FROM EMPLOYEE
...
SELECT *
FROM EMPLOYEE
WHERE EMP_NAME LIKE '전%'; --퍼센트가 의미하는건 어쩌고 저쩌고 전%뒤에 있는 거
...
--전화번호 4번째 자리가 9로 시작하는 사원의 사원명 전화번호조회
SELECT EMP_NAME , PHONE
FROM EMPLOYEE
WHERE PHONE LIKE '___9%';

-- 이메일 중  _앞 글자가 3자리인 이메일 주소를 가진 사원의 사번 사원명, 이메일 조회
SELECT *
FROM EMPLOYEE
