...
--쿼리는 대소문자 구분 X(소문자 작성시 에러 가능성 높아짐. 대문자로 작성하기)
select *
from employee;
...
-- ***산술 연산중 NULL 값이 존재할 경우, 결과값은 무조건 NULL
...
/*
    ...
    -WHY? 산술연산 등으로 인해 칼럼명이 지저분해질 경우, 깔끔하게 작성하기 위해서 사용
*/

/*
    <리터럴>
    SELECT 절에 리터럴을 사용하면 테이블에 존재하는 데이터처럼 조회가 가능하다.
    리터럴은 RESULT SET의 모든 행에 반복적으로 출력된다.(문자열은''사용)
*/
...
--사번,사원명,급여 연결해서 조회(||)
SELECT EMP_ID || EMP_NAME || ' 의 월급은' || SALARY || '원 입니다' AS "급여"
FROM EMPLOYEE;
...
--부서코드 D6이거나 D2에 해당하는 직원의 모든 칼럼 조회
SELECT *
FROM EMPLOYEE
...
--메일 중 _ 앞 글자가 3자리인 이메일 주소를 가진 사원의 사번 사원명, 이메일 조회
SELECT *
FROM EMPLOYEE
