...
, SUBSTR(EMP_NO ,1,8) || '******' FROM EMPLOYEE;

SELECT EMP_NAME , EMP_NO
, RPAD(SUBSTR(EMP_NO ,1,8) , 13 , '*')
FROM EMPLOYEE;
-- EMPLOYEE 테이블에서 사원명, 이메일, 아이디(이메일에서 '@' 앞의 문자 값만 출력)를 조회
SELECT EMP_NAME , EMAIL
, SUBSTR(EMAIL ,1, INSTR(EMAIL , '@' ,1)-1)아이디 FROM EMPLOYEE;
...
SELECT TO_CHAR(SYSDATE, 'MON DY, YYYY') FROM DUAL; --DY(수)/ DAY(수요일)
SELECT TO_CHAR(SYSDATE, 'YYYY-MM-DD(DY)') FROM DUAL; --한글은 인식이 안될수 있으니 ""사용
...
-- 문자 --> 날짜
SELECT TO_DATE('20211014') FROM DUAL;
SELECT TO_DATE('20211014 190830') FROM DUAL;--에러
SELECT TO_DATE('20211014 190830' , 'YYYYMMDD HH24MISS') FROM DUAL;--이런식으로
...
SELECT
EMP_NAME,JOB_CODE
,DECODE(JOB_CODE,'J1','개발1팀','J2','개발2팀')AS 직급명
FROM EMPLOYEE;
...
--(급여가 300 까지는 초급, 500까지는 중급, 700까지는 고급,그 외에는 특급)
SELECT 
EMP_NAME ,SALARY
, CASE
...
-- WHEN EMP-NO LIKE '_______1%' THEN '남자'
ELSE '외국인'
END AS 성별
FROM EMPLOYEE;
